...
// JS: SecureHttpClient.post(url, payload): string
Value PostWrapped(const CallbackInfo& info) {
    Env env = info.Env();
    try {
        ...
        std::string url = info[0].As<String>().Utf8Value();
        std::string payload = info[1].As<String>().Utf8Value();
        std::string result;

        result = SecureHttpClient::post(url, payload);
...
Object Init(Env env, Object exports) {
    try {
        ...
        exports.Set("post", Function::New(env, PostWrapped));
        return exports;
    } catch (const std::exception& e) {
        throw Napi::Error::New(env, e.what());
    } catch (...) {
